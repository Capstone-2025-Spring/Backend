...
public class EvaluationParserService {
    ...
    public EvaluationResultDTO parse(String rawText) {
        ...
        // ✅ Overall Score
        Pattern overallScorePattern = Pattern.compile("#####\\s*Overall Teaching Ability Score\\s*:\\s*(\\d+)");
        ...
        Pattern overallReasonPattern = Pattern.compile(
                "#####\\s*Overall Teaching Ability Score\\s*:\\s*\\d+\\s*\\n@@@@@\\s+([\\s\\S]*?)$",
                Pattern.MULTILINE
        );
        ...
        // ✅ 항목별 점수 + 이유 파싱 (대괄호 유무 허용)
        Pattern itemPattern = Pattern.compile(
                "#####\\s*(?:\\[(.+?)\\]|(.+?))\\s*:\\s*(\\d+)\\s*\\n@@@@@\\s+([\\s\\S]*?)(?=#####\\s*(?:\\[.+?\\]|Overall Teaching Ability Score|.+?)\\s*:\\s*\\d+|\\z)",
                Pattern.MULTILINE
        );
        Matcher itemMatcher = itemPattern.matcher(rawText);

        while (itemMatcher.find()) {
            String name = itemMatcher.group(1) != null ? itemMatcher.group(1).trim() : itemMatcher.group(2).trim();  // 대괄호 있는 이름 또는 없는 이름
